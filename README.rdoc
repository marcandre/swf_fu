...
swf_fu uses SWFObject 2.1 to embed swf objects in HTML and supports all its options.
...
swf_fu has been tested with rails up to v 2.2 and has good test coverage so <tt>rake test:plugins</tt> should reveal
any incompatibility. Comments and pull requests welcome: http://github.com//marcandre/swf_fu
Bug reports can be filed #todo
...
swf_fu from http://github.com/marcandre/swf_fu and install it manually:

  script/plugin install path/to/swf_fu
