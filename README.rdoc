...
+swf_fu+ has been tested with rails v2.0 up to v3.0 and has decent test coverage so <tt>rake test:plugins</tt> should reveal any incompatibility. For Rails 3.1, use the gem[http://github.com/marcandre/swf_fu] instead! Comments and pull requests welcome: http://github.com/marcandre/swf_fu
...
Assuming you have git[http://git-scm.com/] installed (check with <tt>git version</tt>), it is easy to install from your applications directory:
  ...
  rails plugin install git://github.com/marcandre/swf_fu.git -r v1.x   # rails 3

  script/plugin install git://github.com/marcandre/swf_fu.git -r v1.x  # rails 2 (starting at 2.0.2)

For older versions of +rails+ or without +git+, you can always download
+swf_fu+ from github[http://github.com/marcandre/swf_fu/archives/v1.x] and then install it manually:
